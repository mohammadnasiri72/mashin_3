...
function SidebarPodcasts({podcasts}:{podcasts:Items[]}) {
  return (
    <>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="space-y-4">
                {podcasts.map((podcast) => (
                  <div key={podcast.id} className="block group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-xs text-gray-400 mt-2 group-hover:text-white!">
                          <div className="flex items-center gap-1">
                            <FaCalendar />
                            <span>{formatPersianDate(podcast.created)}</span>
                          </div>

                          <div className="flex items-center gap-1">
                            <FaEye className="w-3 h-3" />
                            <span>{toPersianNumbers(podcast.visit)} بازدید</span>
                          </div>
                        </div>
                      </div>
                    </div>
