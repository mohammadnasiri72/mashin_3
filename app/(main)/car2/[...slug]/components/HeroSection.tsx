  ...
  return (
    <section className="relative w-full overflow-hidden" dir="rtl">
      ...
      <div className="absolute inset-0">
        <img
          src={mainDomain + bannerSrc}
          alt={`${detailsCar.sourceName} ${detailsCar.title}`}
          className="object-cover object-center w-full h-full bg-no-repeat"
        />
        {/* Gradient overlays for legibility */}
        <div className="absolute inset-0 bg-linear-to-l from-slate-950/30 via-transparent to-transparent" />
      </div>

      {/* Finance / loan ribbon badge */}
      {detailsCar.summary && (
        <div className=" right-6 top-6 z-10 flex items-center gap-1 rounded-md bg-[#ce1a2a]/95 px-3 py-1.5 text-xs font-semibold text-white shadow-lg">
          {detailsCar.summary}
        </div>
      )}
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-8 h-110 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex max-w-xl flex-col items-start justify-between gap-4 h-full py-10">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            >
              <FaHeart fontSize="small" className="text-[#ce1a2a]" />
