...
export const getItemFindByTerm = async (data: ItemFindByTermParams): Promise<ItemsFindByTerm[]> => {
  try {
    const url = new URL(`${baseUrl}api/Item/findByTerm`);
    ...
    const responseData: ItemsFindByTerm[] = await response.json();
    return responseData;
  } catch (error) {
